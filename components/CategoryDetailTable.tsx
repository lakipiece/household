...
export default function CategoryDetailTable({ allExpenses }: Props) {
  ...
  const detailExpenses = selectedDetail
    ? filtered.filter(e => (e.detail || '기타') === selectedDetail).sort((a, b) => b.date.localeCompare(a.date))
    : []
            ...
            {rows.map((row, i) => {
              ...
              return (
                <Fragment key={i}>
                  ...
                  {isSelected && detailExpenses.map((e, j) => (
                    <tr key={`d-${j}`} className="bg-slate-50 border-b border-slate-100">
                      <td className="py-1.5 pl-8 pr-3 text-xs text-slate-400">{e.date}</td>
                      <td className="py-1.5 px-3 text-right text-xs text-slate-600">{formatWonFull(e.amount)}</td>
